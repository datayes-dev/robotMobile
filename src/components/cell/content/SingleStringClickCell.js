	...
	render() {
		...
		if(contents != null && contents.length > 0) {
			...
			for(var i = 0; i < contents.length; i++) {
				...
				if (moreFlag) {
					tabsJXS[i] =<li key={i} value={i} onClick = {this.onTabClicked}><h3>{contents[i]}</h3><nt></nt></li>;
				} else{
					tabsJXS[i] =<li key={i} value={i} onClick = {this.onTabClicked}><h3>{contents[i]}</h3></li>;
				}
			}
